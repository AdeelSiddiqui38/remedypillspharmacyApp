...
export async function registerRoutes(
  ...
  // The real privacy policy is the client route at /privacy: it is written
  // against Alberta's Health Information Act and PIPEDA and states the 10-year
  // ACP retention rule, none of which the page that used to live here did.
  // Kept as a redirect because this path was submitted to the Play Console.
  ...
  // ── Public Website Integration: Contact Form Webhook ──────
  // Receives "Contact Us" submissions from the WordPress/Elementor site at
  // remedypills.ca (configured as an Elementor Forms "Webhook" action) and
  ...
  app.post("/api/integrations/contact-webhook", messagingLimiter, async (req, res) => {
    ...
    // Elementor's field keys vary by form configuration (auto-generated IDs
    // like "field_1"), so match common field names case-insensitively rather
    // than relying on exact keys.
    const findField = (...candidates: string[]): string | undefined => {
      ...
      for (const candidate of candidates) {
        ...
        if (match && typeof match[1] === "string" && (match[1] as string).trim()) return match[1] as string;
      }
      return undefined;
    };

    const name = findField("name") || "Unknown";
    const email = findField("email");
    const phone = findField("phone", "contact_number", "contact number");
    const message = findField("message", "comment");

    const summaryParts = [name, phone, email].filter((v): v is string => !!v);
    const body = `${summaryParts.join(" · ")}${message ? `: ${message}` : ""}`.slice(0, 500);
    ...
    await storage.createAuditLog({
      ...
      details: `Contact form submission from ${name}${email ? ` (${email})` : ""} via remedypills.ca`,
      ipAddress: req.ip || "unknown",
      timestamp: new Date().toISOString(),
    });
