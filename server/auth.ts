...
export function setupAuth(app: Express) {
  ...
  // Enhanced Google OAuth error handling with detailed error messages
  if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
    ...
    app.get("/api/auth/google/callback", (req, res, next) => {
      passport.authenticate("google", (err: any, user: any, info: any) => {
        if (err) {
          ...
          return res.redirect("/auth?error=google_error&message=" + encodeURIComponent(err.message));
        }
        if (!user) {
          console.warn("Google callback failed:", info);
          const errorMessage = info?.message || "Google authentication failed";
          return res.redirect("/auth?error=google_failed&message=" + encodeURIComponent(errorMessage));
        }
        req.login(user, (loginErr: any) => {
          if (loginErr) {
            console.error("Login after Google failed:", loginErr);
            return res.redirect("/auth?error=google_login&message=" + encodeURIComponent(loginErr.message));
          }
